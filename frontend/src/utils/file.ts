...
export async function getPDFPageCount(file: File): Promise<number> {
  const document = await loadPDFDoc(file);
...
export type PDFPageGroup = { label: string; pages: number[] };

export type ValidationError = { message: string };

export function validatePageExpression(expression: string, totalPages: number): ValidationError | null {
  const trimmed = expression.trim();
  if (!trimmed) return { message: 'Enter at least one page or range.' };
  const values = trimmed.split(',').map((v) => v.trim()).filter(Boolean);
  if (!values.length) return { message: 'Enter at least one page or range.' };
  for (const value of values) {
    ...
    if (range) {
      ...
      if (from < 1 || to > totalPages || from > to) return { message: `Range "${value}" is invalid.` };
    } else if (!single) {
      return { message: `"${value}" is not a valid page number or range.` };
    } else {
      const page = Number(value);
      if (page < 1 || page > totalPages) return { message: `Page ${page} is outside the PDF (1–${totalPages}).` };
    }
  }
  return null;
}

export function validatePageRanges(expression: string, totalPages: number): ValidationError | null {
  const trimmed = expression.trim();
  if (!trimmed) return { message: 'Enter at least one page range.' };
  const lines = trimmed.split(/[\n,]+/).map((v) => v.trim()).filter(Boolean);
  if (!lines.length) return { message: 'Enter at least one page range.' };
  const occupied = new Set<number>();
  for (const line of lines) {
    const range = line.match(/^(\d+)\s*-\s*(\d+)$/);
    if (!range) return { message: `Each line must be a range like "1-5". Found "${line}".` };
    const from = Number(range[1]);
    const to = Number(range[2]);
    if (from < 1 || to > totalPages || from > to) return { message: `Range "${line}" is invalid.` };
    for (let p = from; p <= to; p++) {
      if (occupied.has(p - 1)) return { message: `Page ${p} appears in multiple ranges.` };
      occupied.add(p - 1);
    }
  }
  return null;
...
export function parsePDFPageExpression(expression: string, totalPages: number): number[] {
  const values = expression.split(',').map((value) => value.trim()).filter(Boolean);
  if (!values.length) throw new Error('Enter at least one page or range.');
  ...
  for (const value of values) {
    ...
    if (range) {
      const from = Number(range[1]);
      const to = Number(range[2]);
      if (from < 1 || to > totalPages || from > to) throw new Error('Invalid range: ' + value);
      for (let page = from; page <= to; page += 1) pages.add(page - 1);
    } else if (single) {
      const page = Number(value);
      if (page < 1 || page > totalPages) throw new Error('Page ' + page + ' is outside this PDF.');
      pages.add(page - 1);
    } else {
...
export function createPDFPageGroups(
  totalPages: number,
  every: number,
  pagesLabel = 'Pages'
): PDFPageGroup[]  {
  if (!Number.isInteger(every) || every < 1) throw new Error('Enter a valid number of pages.');

  return Array.from({ length: Math.ceil(totalPages / every) }, (_, index) => {
    const start = index * every;
    const pages = Array.from(
      { length: Math.min(every, totalPages - start) },
      (_, offset) => start + offset
    );

   return {
    label: `${pagesLabel} ${start + 1}-${pages[pages.length - 1] + 1}`,
    pages,
    };
  });
}
...
export function parsePDFPageRanges(
  ...
  if (!values.length) throw new Error('Enter at least one page range.');

  const occupied = new Set<number>();

  return values.map((value) => {
    const pages = parsePDFPageExpression(value, totalPages);
    if (pages.some((page) => occupied.has(page)))
      throw new Error('Ranges cannot overlap: ' + value);

    pages.forEach((page) => occupied.add(page));

    return {
  label: `${pagesLabel} ${pages[0] + 1}-${pages[pages.length - 1] + 1}`,
  pages,
};
  });
}
async function createPDFsForGroups(file: File, groups: PDFPageGroup[]) {
  const source = await loadPDFDoc(file);
...
export async function splitEveryPage(file: File, totalPages: number) {
  return createPDFsForGroups(file, createPDFPageGroups(totalPages, 1));
}
...
export async function splitEveryNPages(
  ...
  return createPDFsForGroups(file, createPDFPageGroups(totalPages, every));
}
...
export async function splitByRanges(
  ...
  return createPDFsForGroups(file, parsePDFPageRanges(ranges, totalPages));
}
...
export async function extractPages(
  ...
  return createPDFsForGroups(file, [
    {
      label: 'Extracted pages',
      pages: parsePDFPageExpression(pages, totalPages),
    },
  ]);
}
