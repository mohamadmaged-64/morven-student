...
function SpeechToTextTool() {
  ...
  const handleFileUpload = useCallback((files: { file: File; data: ArrayBuffer | string }[]) => {
    if (files.length > 0) {
      ...
      addNotification('تم تحميل الملف الصوتي. اضغط "تفريغ الصوت المرفوع" للبدء.', 'info');
    }
  }, [addNotification]);

  const startRecording = useCallback(async () => {
    if (!supported) {
      addNotification('التعرف على الكلام غير مدعوم في متصفحك. جرّب Chrome أو Edge.', 'error');
      return;
    }
    try {
      ...
      if (SpeechRecognitionAPI) {
        ...
        recognition.onerror = (event: SpeechRecognitionErrorEvent) => {
          if (event.error !== 'aborted') {
            addNotification('خطأ في التعرف: ' + event.error, 'error');
          }
      ...
      addNotification('بدأ التسجيل. تحدث بوضوح في الميكروفون.', 'info');
    } catch {
      addNotification('تم رفض الوصول إلى الميكروفون. يرجى السماح بالوصول.', 'error');
    }
  ...
  const stopRecording = useCallback(() => {
    ...
    setIsRecognizing(false);
    addNotification('تم إيقاف التسجيل.', 'success');
  }, [addNotification]);

  const transcribeUpload = useCallback(async () => {
    if (!uploadedAudio) return;
    if (!supported) {
      addNotification('ميزة التعرف على الكلام غير مدعومة. جرّب Chrome أو Edge.', 'error');
      return;
    }
    ...
    try {
      ...
      addNotification('يتطلب التفريغ عبر واجهة Speech API تشغيل الملف الصوتي، وسيتم تشغيله في الخلفية.', 'info');
      ...
      if (!SpeechRecognitionAPI2) {
        addNotification('ميزة التعرف على الكلام غير مدعومة', 'error');
        setProgress(0);
        return;
      }
      ...
      recognition.onend = () => {
        ...
        setProgress(100);
        addNotification('اكتمل تفريغ النص!', 'success');
      ...
      addNotification('حدث خطأ أثناء تفريغ الصوت.', 'error');
      setProgress(0);
    }
  ...
  const handleDownload = useCallback(() => {
    ...
    addNotification('تم تنزيل النص.', 'success');
  }, [transcription, addNotification]);
    ...
    <div className="space-y-6" dir="rtl">
      {!supported && (
        <Card padding="md" className="bg-red-50 dark:bg-red-900/20 border-red-200 dark:border-red-800">
          <p className="text-sm text-red-700 dark:text-red-300">
            <strong>المتصفح غير مدعوم.</strong>
            </p>
        </Card>
      )}

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Select label="اللغة" options={LANGUAGES} value={language} onChange={(e) => setLanguage(e.target.value)} />
        <div className="flex items-end">
          <Badge variant={isRecording ? 'danger' : 'secondary'} dot={isRecording}>
            {isRecording ? `جارٍ التسجيل ${formatTime(recordTime)}` : 'جاهز'}
          </Badge>
          {isRecognizing && <Badge variant="success" className="ms-2">جارٍ التعرف...</Badge>}
        </div>
      </div>
      ...
      <div className="text-center">
        <p className="text-sm text-gray-500 dark:text-gray-400">
          {isRecording
            ? 'اضغط الزر لإيقاف التسجيل.'
            : 'اضغط الزر لبدء التسجيل.'}
        </p>
      </div>

      <div className="border-t border-light-border dark:border-dark-border pt-4">
        <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">أو ارفع ملفًا صوتيًا:</p>
        <FileUpload
          accept={['audio/*']}
          onFilesSelected={handleFileUpload}
          label="ارفع ملفًا صوتيًا"
          description="يدعم MP3 وWAV وOGG وWebM وغيرها من صيغ الصوت."
          maxFiles={1}
        />
      </div>
      ...
      {uploadedAudio && (
        <div className="space-y-3">
          ...
          <div className="flex gap-2">
            <Button onClick={transcribeUpload} loading={isRecognizing} variant="secondary" className="flex-1">تفريغ الصوت المرفوع</Button>
          </div>
        </div>
      )}

      {progress > 0 && <ProgressBar value={progress} color="gradient" label="جارٍ المعالجة..." showLabel />}

      {transcription && (
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="space-y-3">
          <div className="flex items-center justify-between">
            <label className="text-sm font-medium text-gray-700 dark:text-gray-300">النص المفرغ</label>
            <div className="flex gap-2">
              <Badge variant="info">{cleanTranscription.split(/\s+/).filter(Boolean).length} كلمة</Badge>
            </div>
          </div>
          <TextArea
            value={transcription}
...
function AudioToolPage({ toolId }: { toolId: string }) {
  ...
  const configs: Record<ToolId, { title: string; description: string; icon: JSX.Element; component: JSX.Element }> = {
    'speech-to-text': {
      ...
      description: 'حوّل الكلام والملفات الصوتية إلى نص مكتوب.',
