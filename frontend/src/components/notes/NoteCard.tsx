...
function formatNoteDate(timestamp: number, language: Language): string {
  return new Date(timestamp).toLocaleDateString(language === 'ar' ? 'ar-SA' : 'en-US', {
    month: 'short',
    day: 'numeric',
    ...(language === 'en' ? { year: 'numeric' as const } : {}),
  });
}

function NoteCard({ note, language, onEdit, onDelete, onTogglePin }: NoteCardProps) {
    ...
    >
      ...
      >
        <div className="flex items-start gap-2">
          ...
          >
            {note.title.trim() || (language === 'ar' ? 'بدون عنوان' : 'Untitled')}
          </h3>
          <button
            onClick={(e) => {
              e.stopPropagation();
            ...
            title={language === 'ar' ? (note.pinned ? 'إلغاء التثبيت' : 'تثبيت') : note.pinned ? 'Unpin' : 'Pin'}
          >
        ...
        <div className="mt-3 pt-3 border-t border-light-border dark:border-dark-border flex items-center justify-between gap-2">
          <div className="flex items-center gap-3 text-[11px] text-gray-400 dark:text-gray-500">
            <span className="inline-flex items-center gap-1" title={language === 'ar' ? 'تاريخ الإنشاء' : 'Created'}>
              <Calendar className="w-3 h-3" />
              {formatNoteDate(note.createdAt, language)}
            </span>
            <span className="inline-flex items-center gap-1" title={language === 'ar' ? 'آخر تعديل' : 'Last edited'}>
