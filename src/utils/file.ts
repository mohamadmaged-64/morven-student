  ...
  for (const line of lines) {
    ...
    while (remaining.length > 0) {
      ...
      for (const word of words) {
        testLine = lineText ? `${lineText} ${word}` : word;
        if (font.widthOfTextAtSize(testLine, fontSize) > maxWidth && lineText) {
          break;
        }
        lineText = testLine;
      }
...
export async function excelToPDF(file: File): Promise<Blob> {
  ...
  for (const sheetName of workbook.SheetNames) {
    ...
    const numCols = Math.max(...data.map((row) => row?.length || 0));
    ...
    for (const row of data) {
      ...
      for (let c = 0; c < cells.length; c++) {
        ...
        const truncated = cellText.length > 30 ? cellText.slice(0, 27) + '...' : cellText;
...
export async function pptToPDF(file: File): Promise<Blob> {
  ...
  for (const slideFile of slideFiles) {
    ...
    for (let i = 0; i < texts.length; i++) {
      const content = texts[i].textContent?.trim();
    ...
    for (const text of slideTexts) {
      if (y < margin) break;
      const maxChars = Math.floor((pageWidth - margin * 2) / 7);
      const wrappedLines = text.match(new RegExp(`.{1,${maxChars}}`, 'g')) || [text];
